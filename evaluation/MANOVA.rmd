...
initial_descriptive_stats <- map_dfr(1:4, function(treatment) {
  ...
  treatment_median <- treatment_subset_combined %>% 
    ...
    colnames(transponed_median) <- c("treatment_median")
  ...
  treatment_mode <- treatment_subset_combined %>% 
    ...
    colnames(transponed_mode) <- c("treatment_mode")
  ...
  treatment_min <- treatment_subset_combined %>% 
    ...
    colnames(transponed_min) <- c("treatment_min")
  ...
  treatment_max <- treatment_subset_combined %>% 
    ...
    colnames(transponed_max) <- c("treatment_max")
  ...
  output <- cbind(output_mean, output_sd, output_var, output_n, output_treatment_se, output_treatment_lower_CI, output_treatment_higher_CI, transponed_median, transponed_mode, transponed_min, transponed_max)
...
The above calculated table has to be cleaned up for interpretation and for depicting the boxplots by treatment and factor.

```{r}
descriptive_stats <- as.data.frame(initial_descriptive_stats)
descriptive_stats <- dplyr::select(descriptive_stats, -13)
