...
png("latex_exports/polychoric_corr.png", width = 750, height = 750, res = 100)
...
We analyze the absolute fit indices (chi-square, degrees of freedom (df), p-value, standardized root mean square (SRMS)), the parsimony correction index (root mean square error of approximation (RMSEA)), and the comparative fit indices (comparative fit index (CFI), Tucker-Lewis index (TLI)) to evaluate the null hypothesis. In addition, we inspect the residual correlations and modification indices in order to inspect which items can be dropped. We conclude by interpreting the parameter estimates to finally shed a light on how the respecified model looks like. (Brown, 2015)
...
```{r}
mod_ind <- lavaan.mi::modificationIndices.mi(ordinal_CFA1, pool.method = "D2", standardized = TRUE, cov.std = TRUE, sort. = TRUE, minimum.value = 4)
mod_ind
```
...
```{r}
fit_measures_ordinal_CFA2 <- fitMeasures(ordinal_CFA2, pool.method = "D2", fit.measures = c("chisq.scaling.factor", "chisq.scaled", "df", "df.scaled", "pvalue.scaled", "baseline.chisq.scaled", "baseline.df", "baseline.df.scaled", "baseline.pvalue.scaled", "srmr", "rmsea.robust", "rmsea.ci.lower.robust", "rmsea.ci.upper.robust", "rmsea.ci.level", "cfi.robust", "tli.robust"))

df_fit_measures_ordinal_CFA2 <- as.data.frame(round(fit_measures_ordinal_CFA2, 4))
df_fit_measures_ordinal_CFA2
```


For the absolute fit indices, the changes of the scaled chi-square value have to be noted firstly as it has decreased from ≈ 217.856 to ≈ 90.564 significantly which already is a sign for an improved fit. The latter is underscored through the comparison of the scaled model chi-square value (≈ 90.564) and the scaled baseline model chi-square value which is considerably higher with ≈ 204.103. Further, the model has a lowered df = 48, whereas the baseline has df = 66. Even though the fit characteristics have improved at this point, the null hypothesis is rejected with a scaled p-value ≈ 0.0002.

The SRMR of ≈ 0.0895 is closer to the benchmark of 0.08 than for the initial model with ≈ 0.175. As ordinal data on the basis of imputed data is inspected, we accept this value as approximately close to the benchmark moderate fit. To further check on this, the residual correlations are inspected later on.
...
```{r}
mod_ind_refit <- lavaan.mi::modificationIndices.mi(ordinal_CFA2, pool.method = "D2", standardized = TRUE, cov.std = TRUE, sort. = TRUE, minimum.value = 4)
mod_ind_refit
```
...
<h2>Final Reliability Check of the Four Factor Model</h2>
Finally, the score reliability of the final model is evaluated as methods such as (M)ANOVA are based on a perfect reliability assumption (Brown, 2015). Score reliability refers to "the degree to which scores [...] are precise" (Kline, 2016, p. 90), i.e., the degree to which the sum of items actually measure the factor (Kline, 2016). In the following two reliability measures are calculated: Cronbach's alpha and the composite reliability measure omega (Kline, 2016; Jorgensen et al., 2025). Values close to 0 indicate that an dimensions score are likely random and thus, not measuring anything (Kline, 2016). The theoretical value range assumes values between 0-1 (Kline, 2016.

Alpha and omega allow to measure if the factors actually measure what they aim to measure across the model (Kline, 2016). As Cronbach's alpha is often associated as "a misestimator of scale reliability" (Brown, 2015, p. 309), it is supported by omega which also takes the violation of tau equivalence (Brown, 2015), and the item response format of an ordinal scale (Carifio and Perla, 2007; Jorgensen et al., 2025) into account.
...
```
```{r}
omega <- compRelSEM(ordinal_CFA2, tau.eq = FALSE, ord.scale = TRUE)
omega
```

For accountability, a moderate alpha ≈ 0.63, omega ≈ 0.66 indicate a likely moderate reliability. In contrast, disclosure shows a high alpha ≈ 0.8419861, omega ≈ 0.951 which is likely to point to a too high similarity of items. However, as already discussed before, we still stay with this redefined dimension of disclosure due to the literatic distinction of items. For usefulness (alpha ≈ 0.70, omega ≈ 0.72) and explainability (alpha ≈ 0.71, omega ≈ 0.768) also a moderate reliability is assumed. Overall these values indicate that the at hand items of the redefined model precisely measure their respective dimensions which lays the foundation for further analysis. (Streiner, 2003).


<h1>Save Results</h1>
For an increased reproducibility, all CFAs are stored additionally.
...
```{r}
final_items = c("AC01_01", "AC01_02", "AC01_03", "DI01_02", "DI01_03", "DI01_04", "EX01_04", "UI01_01", "UI01_02", "EX01_02", "EX01_03", "UI01_04")

ordinal_CFA2_loadings <- round(est_refit$est[1:12], 4)

accountability <- rep("-", 12)
disclosure     <- rep("-", 12)
usefulness     <- rep("-", 12)
explainability <- rep("-", 12)

accountability[1:3] <- ordinal_CFA2_loadings[1:3]
disclosure[4:7] <- ordinal_CFA2_loadings[4:7]
usefulness[8:9] <- ordinal_CFA2_loadings[8:9]
explainability[10:12] <- ordinal_CFA2_loadings[10:12]

measurement_model_loadings_reliability <- data_frame(
  items = final_items,
...
measurement_model_loadings_reliability <- rbind(measurement_model_loadings_reliability, c("Cronbach's", round(ac_alpha_values_avg, 4), round(di_alpha_values_avg, 4), round(ui_alpha_values_avg, 4), round(ex_alpha_values_avg, 4)))

measurement_model_loadings_reliability <- rbind(measurement_model_loadings_reliability, c("Composite Reliability", round(omega, 4)))

measurement_model_loadings_reliability
```

```{r}
xtable::xtable(measurement_model_loadings_reliability, type = "latex") 
```
